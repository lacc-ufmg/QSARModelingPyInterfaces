...
class CrossValidation(object):
    ...
    def returnParameters(self,nLV = None):
        ...
        r2cal = (calcR(np.reshape(scaledy,len(scaledy)),np.reshape(scaledycal,len(scaledy))))**2
        ...
        r2cv = (calcR(np.reshape(scaledy,len(scaledy)),np.reshape(scaledycv,len(scaledy))))**2
