...
if __name__ == '__main__':
    ...
    # external validation
    dfConf = pd.read_csv(
        "/home/helitonmrf/OneDrive/Documentos/QSAR/Amostragem por Fecho Convexo/Descritores 4D/matrizes_C_LJ/confExtVal.csv", header=None)
    for runIndex in range(1, dfConf.shape[1]):
        print("Starting external validation {} of {}.".format(
            runIndex, (dfConf.shape[1]-1)))
        ...
        dfX = lj.transform(
            dfX) if dfConf[runIndex][13].upper() == "YES" else dfX
        X = dfX.values
        ...
        dfytrain.to_csv(os.path.join(
            out_directory, ytrain_file), sep=',', header=False)
