...
class CrossValidation(object):
    ...
    def returnParameters(self, nLV=None):
        ...
        r2cal = (calcR(np.reshape(scaledy, len(scaledy), 1),
                       np.reshape(scaledycal, len(scaledy), 1)))**2
        ...
        r2cv = (calcR(np.reshape(scaledy, len(scaledy), 1),
                      np.reshape(scaledycv, len(scaledy), 1)))**2
