...
class ExternalValidation(object):
	...
	def extVal(self,train,test,nLV=None):
		...
		Q2F395 = 1-((self.y[new_test]-yp).T.dot(self.y[new_test]-yp)/len(yp))/((ytrain-ytrain.mean()).T.dot(ytrain-ytrain.mean())/len(ytrain))
		...
		r2 = (calcR(np.reshape(scaledy_test,len(scaledy_test)),np.reshape(scaledyp,len(scaledy_test))))**2
