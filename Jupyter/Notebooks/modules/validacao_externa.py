...
from calculate_parameters import ssy,calcPress,calcR,calcR2,calcMAE,calcRMSE
import numpy as np
import pandas as pd
import math
import sys
...
class ExternalValidation(object):
	...
	def validateExtVal(self,train,test,nLV=None):
			...
			higher_residuals = np.argsort(-abs(residuals))[:math.ceil(0.05*len(test))]
	...
	def searchValidExtVal(self, nLV = None,n_test = 10, n_splits = 100):
		...
		for train, test in ss.split(X):
			...
			sd = np.std(abs(residuals),ddof=1)
			# print(sd)
			higher_residuals = np.argsort(-abs(residuals))[:math.ceil(0.05*n_test)]
			higher_residuals = [test[i] for i in higher_residuals]
			new_test = [i for i in test if i not in higher_residuals]
			yp = pls.predict(X[new_test])
			Q2F195 = calcR2(y[new_test],yp,np.mean(y[train]))
	...
	def extVal(self,train,test,nLV=None):
		...
		r2 = (calcR(np.reshape(scaledy_test,len(scaledy_test),1),np.reshape(scaledyp,len(scaledy_test),1)))**2
		...
		dfPred = pd.DataFrame(index = ["Q2F1","Q2F2","Q2F195","Q2F295","k","k'","R0",
										"R0'","dif","AREpred","RMSEP","avgrm",
										"deltarm","MAE","MAE95","test_set","nLV"],
			data=np.array([Q2F1[0],Q2F2[0],Q2F195[0],Q2F295[0],k[0],k1[0],R02[0],
							R102[0],dif[0],AREpred[0],RMSEP,avgrm,deltarm,MAE,MAE95,
							test_set,nLV]))
		return dfPred
	...
	def runValidExtVal(self,directory,n_models = 1):
		...
		for i in range(min(n_models,len(self.tests_sets))):
			...
			dfPred.to_csv(directory+"test"+str(i+1)+".csv", sep =',', header=False)
