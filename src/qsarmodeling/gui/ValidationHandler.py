...
class ValidationHandler(Handler):
    ...
    def on_cv_run_button_clicked(self, _) -> None:
        ...
        nLV = (
            None
            if auto
            else self.builder.get_object("cross_validation_nLV").get_value()
        )
        filename = self.builder.get_object("cv_output").get_text()

        logger.debug("Calling RunCalculations.")
        cv = RunCalculations.runCrossValidation(
        ...
        texts = ResultWindowTexts(
            toptext="Your Cross Validation is done.",
    ...
    def on_yrlno_run_button_clicked(self, _) -> None:
        ...
        passed_str = (
            "YRandomization passed with score {}."
            if yr_result["passed"]
            else "YRandomization failed with score {}.\nEither adjust your desired score or check your data."
        )
        stats = passed_str.format(yr_result["score"])

        texts = ResultWindowTexts(
            toptext="Your Y-Randomization is done.",
    ...
    def on_extval_run_button_clicked(self, _) -> None:
        ...
        basename = os.path.join(
            os.path.dirname(self.handler.get_X_matrix()), f"{X_name}"
        )
        default_outputs = {
        ...
        self.ext_val_config: ConfigExtValInterface = {
            ...
            "latent_vars_model": self.builder.get_object(
                "extval_vars_model"
            ).get_value()
            or None,
            "extval_type": self.builder.get_object("extval_type").get_active_id(),
            "output_extval": self.builder.get_object("extval_output_extval").get_text()
            or default_outputs["output_extval"],
            "output_cv": self.builder.get_object("extval_output_cv").get_text()
            or default_outputs["output_cv"],
            "output_X_train": self.builder.get_object(
                "extval_output_X_train"
            ).get_text()
            or default_outputs["output_X_train"],
            "output_y_train": self.builder.get_object(
                "extval_output_y_train"
            ).get_text()
            or default_outputs["output_y_train"],
            "output_X_test": self.builder.get_object("extval_output_X_test").get_text()
            or default_outputs["output_X_test"],
            "output_y_test": self.builder.get_object("extval_output_y_test").get_text()
            or default_outputs["output_y_test"],
