...
from qsarmodeling.Interfaces import ConfigGAInterface
...
class GAHandler(Handler):
    ...
    def on_GA_run_button_clicked(self, _) -> None:
        if self._is_running():
            logging.warning(f"Already running with PID = {self.running_process.pid}")
            return
        ...
        if self.handler.files_ok():
            self.ga_config = {
                ...
                "output_selected": self.builder.get_object(
                    "GA_output_selected_variables"
                ).get_text(),
                "output_PLS_model": self.builder.get_object(
                    "GA_output_model"
                ).get_text(),
                ...
                "max_latent_model": self.builder.get_object(
                    "ga_max_latent_model"
                ).get_value(),
                "min_vars_model": self.builder.get_object(
                    "ga_min_vars_model"
                ).get_value(),
                "max_vars_model": self.builder.get_object(
                    "ga_max_vars_model"
                ).get_value(),
                "population_size": self.builder.get_object(
                    "ga_population_size"
                ).get_value(),
                "migration_rate": self.builder.get_object(
                    "ga_migration_rate"
                ).get_value(),
                "crossover_rate": self.builder.get_object(
                    "ga_crossover_rate"
                ).get_value(),
                "mutation_rate": self.builder.get_object(
                    "ga_mutation_rate"
                ).get_value(),
            ...
            self.running_process = Process(
                target=GAHandler.call_runner, args=(self.ga_config,)
            )
